...
from app.dao.gop_config_dao import GopConfigDAO
from app.logger import setup_logger
from app.dao.prompt_dao import PromptDAO
from app.utils.api_utils import get_expensive_hosp_info_api
...
def pre_analyze_preauth_result1(apv_info: str) -> Optional[dict]:
    ...
    try:
        ...
        import re
        content = re.sub(r' thinking.*? response', '', content, flags=re.DOTALL)
...
def clean_json_string(content: str) -> str:
    """清理JSON字符串中的非法控制字符、Markdown包装和空白"""
    if not content:
        return content
    # 移除控制字符
